...
//
// Production v0.7 deliberately calls the proven pre-snap v0.6.9 update captured
// by v070.js (`updateV069ForV070`) and audits the resulting continuous movement.
// Historical tests opened with `?test=1` are intentionally left on the exact
// legacy stack they were written against; the production v0.7 tests use the
// non-legacy `test=v070` harness.
...
if (!V070_LEGACY_TEST_MODE) {
  ...
  function allowedMemberStepV070(u, dt) {
    ...
    // small overlap correction. This ceiling is deliberately above legitimate
    // movement, but far below a target-slot teleport.
