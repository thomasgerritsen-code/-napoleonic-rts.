...
(function installStuckRecoveryV2(global){
  ...
  function villageObstacles(){
    ...
    return villages.flatMap(v=>v.houses||[]).map(h=>({id:`v:${h.id||`${h.kind}-${h.x}-${h.y}`}`,x:h.x,y:h.y,w:h.w,h:h.h,angle:h.angle||0,kind:'village'}));
  }
  function gameplayObstacles(){
    return buildings.filter(b=>!b.dead).map(b=>({id:`g:${b.id}`,x:b.x,y:b.y,w:b.w,h:b.h,angle:0,kind:'gameplay'}));
  }
  ...
  function toWorld(p,o){
    const c=Math.cos(o.angle||0),s=Math.sin(o.angle||0);
    return {x:o.x+p.x*c-p.y*s,y:o.y+p.x*s+p.y*c};
  }
  function insideExpanded(p,o,pad=clearance){
    const q=toLocal(p,o);
    return Math.abs(q.x)<o.w*.5+pad && Math.abs(q.y)<o.h*.5+pad;
  }
  function segmentAabbHitLocal(a,b,hw,hh){
    let t0=0,t1=1;
    const dx=b.x-a.x,dy=b.y-a.y;
    const tests=[[-dx,a.x+hw],[dx,hw-a.x],[-dy,a.y+hh],[dy,hh-a.y]];
  ...
  function segmentHitsObstacle(a,b,o,pad=clearance){
    const la=toLocal(a,o),lb=toLocal(b,o),hw=o.w*.5+pad,hh=o.h*.5+pad;
    const startInside=Math.abs(la.x)<hw&&Math.abs(la.y)<hh;
    if(startInside){
      const endInside=Math.abs(lb.x)<hw&&Math.abs(lb.y)<hh;
      if(!endInside){
        const startNorm=Math.max(Math.abs(la.x)/hw,Math.abs(la.y)/hh);
        const endNorm=Math.max(Math.abs(lb.x)/hw,Math.abs(lb.y)/hh);
        if(endNorm>startNorm+1e-4)return false;
      }
    }
    return segmentAabbHitLocal(la,lb,hw,hh);
  }
  function firstBlocker(a,b,ignoreId=null,pad=clearance){
    let best=null,bestD=Infinity;
    for(const o of obstacles()){
      if(o.id===ignoreId)continue;
      if(!segmentHitsObstacle(a,b,o,pad))continue;
      const d=Math.hypot(a.x-o.x,a.y-o.y);
      if(d<bestD){best=o;bestD=d;}
    }
    return best;
  }
  function pointClear(p,ignoreId=null,pad=clearance){
    if(p.x<10||p.y<10||p.x>WORLD.width-10||p.y>WORLD.height-10)return false;
    for(const o of obstacles())if(o.id!==ignoreId&&insideExpanded(p,o,pad))return false;
    return true;
  }
  function projectTargetOutside(p,o,pad=clearance){
    if(!insideExpanded(p,o,pad))return p;
    const q=toLocal(p,o),hw=o.w*.5+pad,hh=o.h*.5+pad;
    const px=hw-Math.abs(q.x),py=hh-Math.abs(q.y);
    if(px<py)q.x=(q.x<0?-1:1)*(hw+.5); else q.y=(q.y<0?-1:1)*(hh+.5);
    return toWorld(q,o);
  }
  function sanitizedTarget(target){
    ...
    for(let pass=0;pass<6;pass++){
      let changed=false;
      for(const o of obstacles()){
        if(!insideExpanded(p,o,clearance))continue;
        p=projectTargetOutside(p,o,clearance);changed=true;
      }
      if(!changed)break;
    }
    return p;
  }
  function cornerCandidates(o){
    const hw=o.w*.5+clearance+cornerClearance,hh=o.h*.5+clearance+cornerClearance;
    return [[-hw,-hh],[-hw,hh],[hw,-hh],[hw,hh]].map(([x,y])=>toWorld({x,y},o));
  }
  function localWaypoint(a,target,unit){
    const safeTarget=sanitizedTarget(target);
    const blocker=firstBlocker(a,safeTarget,null,clearance);
    if(!blocker)return {target:safeTarget,waypoint:null,blocker:null};
    stats.blockedDirectSteps++;
    const candidates=cornerCandidates(blocker).filter(p=>pointClear(p,blocker.id,clearance));
    let best=null,bestCost=Infinity;
    for(const p of candidates){
      if(firstBlocker(a,p,blocker.id,clearance))continue;
      if(firstBlocker(p,safeTarget,null,clearance))continue;
      const cost=Math.hypot(p.x-a.x,p.y-a.y)+Math.hypot(safeTarget.x-p.x,safeTarget.y-p.y);
    ...
    return {target:safeTarget,waypoint:best,blocker};
  }
  ...
  function nudgeLooseUnit(u){
    ...
    const proposed={x:Math.max(12,Math.min(WORLD.width-12,u.x+px*nudge*sign)),y:Math.max(12,Math.min(WORLD.height-12,u.y+py*nudge*sign))};
    if(pointClear(proposed,null,2)){u.x=proposed.x;u.y=proposed.y;u.arrivedAtTarget=false;stats.unitNudges++;}
  }
  ...
  moveToward=function moveTowardWithLocalAvoidanceV2(u,tx,ty,dt,speed=TYPES[u.type].speed){
    ...
    const finalTarget=sanitizedTarget({x:tx,y:ty});
    let state=u.localAvoidanceV2||null;
    if(state){
      const waypointDistance=Math.hypot(u.x-state.x,u.y-state.y);
      const directClear=!firstBlocker({x:u.x,y:u.y},finalTarget,null,clearance);
      if(directClear||waypointDistance<=waypointArrival||elapsed-(state.startedAt??elapsed)>maxWaypointSeconds){
        u.localAvoidanceV2=null;state=null;stats.detourResumes++;
      }
    }
    if(!state){
      const route=localWaypoint({x:u.x,y:u.y},finalTarget,u);
      if(route.waypoint){
        state={x:route.waypoint.x,y:route.waypoint.y,blockerId:route.blocker?.id||null,startedAt:elapsed,finalX:finalTarget.x,finalY:finalTarget.y};
        u.localAvoidanceV2=state;stats.localDetours++;
      }
    }
    if(state){
      previousMoveToward(u,state.x,state.y,dt,speed);
      u.arrivedAtTarget=false;
      return false;
    }
    return previousMoveToward(u,finalTarget.x,finalTarget.y,dt,speed);
  };

  const previousUpdate=update;
  update=function updateWithStuckRecoveryV2(dt){
    previousUpdate(dt);
  ...
  global.__STUCK_RECOVERY_V2__=api;
