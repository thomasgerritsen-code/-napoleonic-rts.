...
(function installRiverCrossingRecoveryV1(global){
  ...
  const axisProgressEpsilon=Math.max(.35,Number(cfg.axisProgressEpsilon)||1.25);
  const recoveryDuration=.95,nearRadius=560,unitContextRadius=300;
  const unitState=new Map(),groupState=new Map();
  const stats={unitRecoveries:0,groupRecoveries:0,axisStallRecoveries:0,blockedTargetRecoveries:0,blockedSegmentContexts:0,bridgeRecoveries:0,fordRecoveries:0,unsafeRecoveryRejected:0,maxUnitStallSeconds:0,maxGroupStallSeconds:0,maxAxisNoProgressSeconds:0,unitContextRecoveries:0};
