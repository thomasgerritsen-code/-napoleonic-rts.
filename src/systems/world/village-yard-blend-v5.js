...
(function installVillageYardBlendV5(global) {
  ...
  const YARD_MULTIPLIERS = Object.freeze({
    cottage:[2.0,2.35], farmhouse:[2.15,2.65], barn:[1.8,2.0], inn:[2.0,2.15], chapel:[1.8,2.5]
  });
  ...
  function openSidesFor(house,village,yard) {
    ...
    // V6 settlement hierarchy deliberately suppresses parcel-box reading.
    if(house.zone==='core') return {left:true,right:true,top:true,bottom:true};
    if(house.zone==='residential') {
      open.left=true;open.right=true;
      open[house.side>0?'top':'bottom']=true; // roadside/front edge remains open to the common lane.
    }
    if(house.zone==='farm-edge') open[house.side>0?'top':'bottom']=true;
    return open;
  }
  ...
  function drawNaturalBoundary(yard,kind,seed,open,zone) {
    ...
    ctx.strokeStyle=kind==='chapel'?'rgba(109,105,91,.48)':kind==='barn'?'rgba(91,72,47,.43)':'rgba(53,78,43,.48)';
  ...
  function drawVillageCommonsV5(village) {
    ...
    for(let i=0;i<houses.length;i++){
      ...
      for(const {b,d} of candidates){
        ...
        ctx.strokeStyle='rgba(118,118,75,.075)';ctx.lineWidth=Math.max(18,Math.min(42,(a.plotRadius||30)*.42));
        ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();links++;
      }
    }
    ctx.restore();
  }
