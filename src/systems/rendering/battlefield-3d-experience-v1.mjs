...
if (!api || !canvas || !modeButton) {
  ...
  function importVisualLayersWhenReady() {
    ...
    import('./battlefield-3d-unit-detail-v1.mjs?build=graphics9').catch(error => {
      console.warn('3D unit detail layer failed to load', error);
    });
    import('./battlefield-3d-selection-feedback-v1.mjs?build=graphics9').catch(error => {
      console.warn('3D selection feedback layer failed to load', error);
    });
    import('./battlefield-3d-combat-feedback-v1.mjs?build=graphics9').catch(error => {
