...
(function installTerrainLOSV1(global){
  ...
  nearestEnemyEntity=function(unit,maxRange){const otherSide=opposite(unit.side);let best=null,bestD2=maxRange*maxRange;for(const other of units){if(other.dead||other.side!==otherSide||other.routing||other.type==='worker')continue;const dx=other.x-unit.x,dy=other.y-unit.y,d2=dx*dx+dy*dy;if(d2<bestD2&&lineOfSight(unit,other)){bestD2=d2;best=other;}}for(const b of buildings){if(b.dead||b.side!==otherSide||!b.complete)continue;const dx=b.x-unit.x,dy=b.y-unit.y,d2=dx*dx+dy*dy;if(d2<bestD2&&lineOfSight(unit,b)){bestD2=d2;best=b;}}return best;};
