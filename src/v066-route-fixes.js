'use strict';
// ---------- v0.6.6 network-aware road routing ----------
// Route long marches through actual road junctions instead of asking the coarse world grid
// to approximate an entire road network. Short tactical moves remain ordinary A* field orders.
...
for(const road of ROAD_NETWORK_V066){
  for(let i=1;i<road.points.length;i++){
    const a=ensureRoadGraphNodeV066(road.points[i-1]), b=ensureRoadGraphNodeV066(road.points[i]);
...
function nearestRoadGraphNodeV066(point,kind){
  ...
  for(const node of ROAD_GRAPH_V066.values()){
    ...
    // Prefer a close node, but allow a nearby chaussée junction to win over a minor track.
...
function roadGraphRouteV066(startNode,goalNode,kind){
  ...
  while(open.size&&iterations++<5000){
    ...
    for(const edge of node.edges){
      ...
      dist.set(edge.to,tentative); previous.set(edge.to,{from:current,edge}); open.add(edge.to);
    }
  }
  ...
  while(key!==startNode.key){
    ...
    const node=ROAD_GRAPH_V066.get(key); steps.push({x:node.x,y:node.y,road:prev.edge.road}); key=prev.from;
  }
  steps.reverse();
...
function buildNetworkRoadCandidateV066(start,goal,kind){
  const startAccess=nearestRoadGraphNodeV066(start,kind), goalAccess=nearestRoadGraphNodeV066(goal,kind);
...
const buildRegimentPathV065ForV066Network=buildRegimentPathV06;
buildRegimentPathV06=function buildRegimentPathV066Network(start,goal){
  ...
  if(!candidate){
    return buildRegimentPathV065ForV066Network(start,goal);
  }
  ...
  // The coarse A* can independently discover part of the road network. Keep it only when it
  // is demonstrably faster, but still classify it as a road march when enough of it is on roads.
...
if(window.__RTS_DEBUG__){
  window.__RTS_DEBUG__.roadGraphStatsV066=()=>({nodes:ROAD_GRAPH_V066.size,edges:[...ROAD_GRAPH_V066.values()].reduce((s,n)=>s+n.edges.length,0)/2});
}
