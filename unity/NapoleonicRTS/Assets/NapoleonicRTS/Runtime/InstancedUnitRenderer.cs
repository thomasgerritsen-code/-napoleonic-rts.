...
namespace NapoleonicRTS.Runtime
{
    public sealed class InstancedUnitRenderer : IDisposable
    {
        ...
        public InstancedUnitRenderer()
        {
            ...
            _franceParams = new RenderParams(_france) { worldBounds = new Bounds(Vector3.zero, new Vector3(220f, 160f, 10f)) };
            _britainParams = new RenderParams(_britain) { worldBounds = new Bounds(Vector3.zero, new Vector3(220f, 160f, 10f)) };
        }

        public void Render(SimulationWorld world, float alpha)
        {
            RenderSide(world, ArmySide.France, alpha, _franceParams);
            RenderSide(world, ArmySide.Britain, alpha, _britainParams);
        }

        private void RenderSide(SimulationWorld world, ArmySide side, float alpha, RenderParams renderParams)
        {
            var count = 0;
