...
test('v1.3.13 exposes the four playability control improvements', () => {
...
test('v1.3.13 playability controls load immediately after base input and publish release identity', () => {
  const input = html.indexOf('src/input.js?build=131a');
  const controls = html.indexOf('src/systems/input/playability-controls-v1.js?build=1313a');
  ...
  assert.match(version, /const VERSION = '1\.3\.13'/);
});
