...
test('v1.3.11 village props stay render-only and expose all five yard-detail features', () => {
  ...
  assert.match(index, /battlefield-3d-village-buildings-v1\.mjs\?build=1310a[\s\S]*battlefield-3d-village-props-v1\.mjs\?build=1311a[\s\S]*battlefield-3d-village-landscape-v1\.mjs/);
  assert.match(version, /const VERSION = '1\.3\.11'/);
