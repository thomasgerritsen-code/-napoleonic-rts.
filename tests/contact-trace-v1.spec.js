...
// MOVEMENT-CONTACT-V1 gameplay-side trace. This deliberately observes intent/state
// only; locomotion and formation execution remain owned by Movement/Simulation.
test('frontal contact trace exposes gameplay handoff and target-retention evidence', async ({ page }) => {
  ...
  await page.waitForFunction(()=>Boolean(window.__RTS_DEBUG__?.createFreshInfantryRegiment&&window.__AI_COMMANDER_V1__&&window.__AI_ORDER_DISCIPLINE_V1322__));
  ...
  const trace=await page.evaluate(()=>{
    ...
    for(const u of units){
      ...
      if(u.side==='france'){u.x=120;u.y=120;u.targetX=120;u.targetY=120;u.morale=100;}
    }
